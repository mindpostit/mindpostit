...
export default function Intro({ setView, user, setPrevView }) {
  ...
  const goLogin = () => {
    if (user && !user.isAnonymous) { setView('home'); }
    else { setPrevView && setPrevView('intro'); setView('login'); }
  };

  const btnFill = { display:'inline-flex', alignItems:'center', justifyContent:'center', padding:'18px 30px', borderRadius:'999px', fontSize:'16px', fontWeight:'800', background:'#23231f', color:'#f8f5ef', border:'none', cursor:'pointer', fontFamily:"'Noto Sans KR', sans-serif", boxShadow:'0 14px 28px rgba(35,35,31,.18)' };
  const btnOutline = { display:'inline-flex', alignItems:'center', justifyContent:'center', padding:'18px 30px', borderRadius:'999px', fontSize:'16px', fontWeight:'800', background:'transparent', color:'#5e5850', border:'1px solid #d9d0c3', cursor:'pointer', fontFamily:"'Noto Sans KR', sans-serif" };
    ...
    <div style={{ fontFamily:"'Noto Sans KR', sans-serif", color:'#262522', background:'linear-gradient(180deg,#f6f1ea 0%,#efe8de 100%)', minHeight:'100vh' }}>
      ...
      <header style={{ position:'sticky', top:0, zIndex:20, background:'rgba(245,240,234,.94)', backdropFilter:'blur(12px)', borderBottom:'1px solid rgba(217,208,195,.85)' }}>
        <nav style={{ maxWidth:'1360px', margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 28px', gap:'16px', flexWrap:'wrap' }}>
          <div style={{ fontFamily:"'Do Hyeon', sans-serif", fontSize:'20px', fontWeight:'400', letterSpacing:'.06em', color:'#7b7368' }}>마인드포스팃</div>
      ...
      <main>
        ...
        <section style={{ minHeight:'calc(100vh - 82px)', display:'flex', alignItems:'center', justifyContent:'center', padding:'54px 24px 64px', background:'radial-gradient(circle at 50% 45%, rgba(255,255,255,.70), rgba(255,255,255,0) 42%)' }}>
          <div style={{ width:'min(100%, 700px)', background:'rgba(255,255,255,.65)', border:'1px solid rgba(255,255,255,.82)', borderRadius:'42px', padding:'48px 42px 36px', boxShadow:'0 28px 80px rgba(38,37,34,.10)', textAlign:'center' }}>
            <div style={{ fontFamily:"'Do Hyeon', sans-serif", fontSize:'16px', fontWeight:'400', letterSpacing:'.06em', color:'#a79c8e', marginBottom:'22px' }}>마인드포스팃</div>
            ...
            <div style={{ display:'flex', justifyContent:'center', gap:'12px', flexWrap:'wrap', margin:'28px 0 24px' }}>
              <button onClick={goWrite} style={btnFill}>지금 남기기</button>
              <button onClick={goLogin} style={btnOutline}>로그인 · 나만의 공간</button>
        ...
        <section id="flow" style={{ padding:'88px 24px' }}>
          <div style={{ maxWidth:'1180px', margin:'0 auto' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(240px, 1fr))', gap:'16px' }}>
              {steps.map(({ num, title, desc, time, cardTitle, cardSub, cardText, btn }) => (
                <div key={num} style={{ background:'linear-gradient(180deg,#fffdfa,#f7efe4)', border:'1px solid #d9d0c3', borderRadius:'28px', padding:'24px', boxShadow:'0 16px 34px rgba(38,37,34,.05)' }}>
                  ...
                  <div style={{ height:'280px', background:'#f3ede4', border:'7px solid #262522', borderRadius:'28px', padding:'12px', overflow:'hidden', boxShadow:'0 18px 32px rgba(38,37,34,.10)' }}>
                    ...
                    <div style={{ background:'#fffdfa', border:'1px solid #d9d0c3', borderRadius:'16px', padding:'16px' }}>
        ...
        {/* 실제 대화 */}
        <section style={{ padding:'88px 24px', background:'rgba(255,255,255,.3)' }}>
          <div style={{ maxWidth:'760px', margin:'0 auto' }}>
            <div style={{ fontSize:'15px', fontWeight:'800', letterSpacing:'.18em', color:'#a29789', marginBottom:'12px' }}>실제 대화</div>
            <h2 style={{ fontSize:'clamp(22px, 3.5vw, 34px)', lineHeight:'1.28', letterSpacing:'-.04em', fontWeight:'900', marginBottom:'28px' }}>이런 느낌으로 이어져요.</h2>
            <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
              <div style={{ marginLeft:'auto', maxWidth:'76%', padding:'16px 18px', borderRadius:'20px', fontSize:'16px', lineHeight:'1.8', background:'#fffdfa', border:'1px solid #d9d0c3', color:'#302d2a', boxShadow:'0 4px 10px rgba(38,37,34,.04)' }}>퇴근하고 누웠는데 머리가 멈추질 않아요. 별일 아닌데 자꾸 커져요.</div>
              <div style={{ maxWidth:'76%', padding:'16px 18px', borderRadius:'20px', fontSize:'16px', lineHeight:'1.8', background:'#262522', color:'#f8f4ed', boxShadow:'0 4px 10px rgba(38,37,34,.10)' }}>하루가 끝났는데도 머리가 계속 달리고 있었던 것 같아요. 여기까지 적어준 것만으로도 조금은 덜 혼자였으면 좋겠어요.</div>
              <div style={{ marginLeft:'auto', maxWidth:'76%', padding:'16px 18px', borderRadius:'20px', fontSize:'16px', lineHeight:'1.8', background:'#fffdfa', border:'1px solid #d9d0c3', color:'#302d2a', boxShadow:'0 4px 10px rgba(38,37,34,.04)' }}>고마워요. 오늘 출근길도 벌써 막막해요.</div>
            </div>
            <p style={{ fontSize:'14px', color:'#a09689', marginTop:'12px', textAlign:'center' }}>진짜 사람이 직접 읽고 남긴 답장이에요.</p>
          </div>
        </section>

        {/* 클로징 */}
        <section style={{ padding:'96px 24px 112px' }}>
          <div style={{ maxWidth:'980px', margin:'0 auto', background:'linear-gradient(180deg,#fffdfa,#f7efe4)', border:'1px solid #d9d0c3', borderRadius:'36px', padding:'48px 32px', textAlign:'center', boxShadow:'0 28px 80px rgba(38,37,34,.10)' }}>
            <h2 style={{ fontSize:'clamp(28px, 5vw, 40px)', lineHeight:'1.3', letterSpacing:'-.05em', fontWeight:'900', marginBottom:'18px' }}>어떤 이야기든,<br />여기선 괜찮아요.</h2>
            <p style={{ fontSize:'18px', lineHeight:'1.9', color:'#6c655d' }}>말하기 어려운 이야기일수록, 조용히 둘 곳이 필요하니까.</p>
            <p style={{ fontSize:'18px', lineHeight:'1.9', color:'#6c655d', marginBottom:'28px' }}>오늘 마음 한 장을 마인드포스팃에 남겨보세요.</p>
            <div style={{ display:'flex', justifyContent:'center', gap:'12px', flexWrap:'wrap' }}>
              <button onClick={goWrite} style={btnFill}>지금 남기기</button>
              <button onClick={() => window.scrollTo({ top:0, behavior:'smooth' })} style={btnOutline}>처음으로</button>
            </div>
          </div>
        </section>
      </main>
